...
```{r}
getwd()
load("C:\\Users\\Daphn\\Documenten\\labjournal\\Data\\twitter_20190919.Rdata") 
...
# The estimate of the retweats within the same party is positive compared to the estimate of the retweats between different parties. So, there is segregation along party affiliation in the retweet network among Dutch MPs.
```
...
myeff_m2 <- includeEffects(myeff_m2, absDiffX, interaction1 = "lft", name = "rtnet")
ansM2 <- siena07(myalgorithm, data = mydata, prevAns = ansM1, effects = myeff_m2, useCluster = TRUE, nbrNodes = 4, initC = TRUE, 
    batch = TRUE)
...
```{r, eval=FALSE}
# I have not (yet) succeeded in estimating the right model.
myeff_m3 <- myeff
